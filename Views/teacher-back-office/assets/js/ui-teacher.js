(function(){
  ...
  const TUI = {
          ...
          <div class="card h-100 shadow-sm">
            <div class="card-body d-flex flex-column">
              ...
              <div class="mt-auto d-flex gap-2">
                <a href="quiz-builder.html?courseId=${escapeHtml(c.id)}" class="btn btn-primary btn-sm">Build Quiz (${courseQuizzes.length})</a>
                <button class="btn btn-outline-danger btn-sm" data-id="${escapeHtml(c.id)}">Delete</button>
              </div>
            </div>
          </div>`;
        el.appendChild(div);
    ...
    renderStudentsTable(containerId, students, scores){
      ...
      const rows = students.map(st=> `<tr><td>${escapeHtml(st.username)}</td><td>${avgPct(st.id)}</td><td>${scores.filter(s=>s.userId===st.id).length}</td></tr>`).join('');
